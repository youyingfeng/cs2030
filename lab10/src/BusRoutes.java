...
class BusRoutes {
    ...
    public CompletableFuture<String> description() {
        ...
        for (BusService service: services.keySet()) {
            ...
            CompletableFuture<String> serviceDescription = describeService(service, stops);
            result = result.thenCombine(serviceDescription,
    ...
    public CompletableFuture<String> describeService(BusService service,
        ...
        return stops.thenApply(set -> set.isEmpty()
                ? ""
                : set.stream()
